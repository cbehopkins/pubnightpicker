...
func (l *Listener) createFact(pollID string) {
	targetPayload, err := cellar.JSONCodec[handlers.PollObservedPayload]().Marshal(handlers.PollObservedPayload{PollID: pollID})
	if err != nil {
	...
	pendingPayload, err := cellar.JSONCodec[firebaseidempotency.PendingPayload]().Marshal(firebaseidempotency.PendingPayload{
		Listener: ListenerNewPoll,
		EventKey: pollID,
		Fanout: []firebaseidempotency.FanoutTarget{{
			HandlerName: handlers.HandlerNewPoll,
			Payload:     targetPayload,
		}},
	})
	if err != nil {
		l.logger.Error("marshal new poll pending payload", "poll_id", pollID, "err", err)
		return
	}
