...
def test_complete_poll_listener_enqueues_comp_poll_event():
    ...
    plugin._complete_poll_event_callback(document)

    event = event_queue.get(timeout=0.1)
    assert event.type == EventType.COMP_POLL
    assert event.doc is document


def test_complete_poll_listener_handler_calls_db_handler():
    db_handler: CompletePollDbHandler = _FakeDbHandler()
    event_queue: JobQueue = JobQueue()
    action_manager: ActionMan = _FakeActionManager()
    plugin = CompletePollListenerPlugin(
    ...
    plugin._complete_poll_handler(document, pubs_list)

    assert cast(_FakeDbHandler, db_handler).calls == [
        (pubs_list, action_manager, "poll-2")
    ]
